...
import FileUpload from '@repo/ui/FileUpload';
...
let test = ()=>{
  ...
  let handleIncomingFile = async (file)=>{
try{
  let formData = new FormData();
  formData.append('file',file)

  console.log(`Event received from the child component`,file)
 let res =  await axios.post('http://localhost:5500',formData)

console.log(`The request received from the backend`, res)

}
catch(error)
{
  console.log(`Error occured while trying to send data to backend`,error)
}
  ...
  <FileUpload onClick = {handleIncomingFile}/>
